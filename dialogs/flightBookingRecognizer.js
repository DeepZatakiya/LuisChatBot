...
class FlightBookingRecognizer {
	...
	getSelectEntities(result) {
        let selectValue, fromCollumnValue;
        if (result.entities.$instance.Select) {
            selectValue = result.entities.$instance.Select[0].text;
        }
        if (selectValue && result.entities.Select[0].Collumn) {
            fromCollumnValue = result.entities.Select[0].Collumn[0][0];
        }

        return { Select: selectValue, collumn: fromCollumnValue };
    }
	
	getWhereEntities(result) {
        let whereValue, fromConditionValue;
        if (result.entities.$instance.Where) {
            whereValue = result.entities.$instance.Where[0].text;
        }
        if (whereValue && result.entities.Where[0].Condition) {
            fromConditionValue = result.entities.Where[0].Condition[0];
        }

        return { Where: whereValue, condition: fromConditionValue };
    }
...
// SIG // sbOxbZcTEomhghLlMIIS4QYKKwYBBAGCNwMDATGCEtEw
...
// SIG // AGN/rwkCBmCumscvjhgTMjAyMTA2MTUyMjQwNTUuNzYx
...
// SIG // 6NPEInFichy3vaqybaf+oZwc4LH4i98T0z3LPXTNhDCB
...
// SIG // BQAEggEAIdEa2IkTRLJ+JQFvcMoYsvAD+xDPIfHn963M
// SIG // uJTN+PWp3tTx1RblqQWy1J4GH+DdUJGo1ZEikByGa6kr
// SIG // mBXwjHo3IJJ2FBHWdKfm/hhSfsidatFNG7PBgLC5ornR
// SIG // PN/531Icfv1IKO9DtoZHR0/OAqykaw3vYXJgCeFYq9mk
// SIG // tJTqCnCfIzGOYx3vMzeH2GZMGBVkVq/UlSQ3l8WPamlo
// SIG // rCvmXgHbgwPlmiC7d0VtMSwQo1yp7blk/1Bu8rcKAUqd
// SIG // 7+hWE8LQz22+IcZXZOSEu3B4gyCohtE0PxPaAncJm3kx
// SIG // M6I4ZIm26kLqsPDfGo7CkS8IeQ2Dn2hVSHsSNyOJKg==
